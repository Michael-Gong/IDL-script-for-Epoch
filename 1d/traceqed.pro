...
script=where((traceid ge 1) and (tracegamma gt 1200) and (tracex gt 55.0e-6) and (tracex lt 80.0e-6) and (abs(tracey) lt 2.5e-6))
arrayid = setintersection(arrayid,script)
print, size(arrayid)
...
for i=start, stop do begin
   ...
   ; fs[i-start] = t.time
    ...
    tracex   = temp.grid_electron.x
    tracey   = temp.grid_electron.y
    tp       = getdata(i,/px_electron,/py_electron,/particle_eta_electron,/gamma_electron,/particle_radn_electron,/particle_radt_electron)
    tracepx  = tp.px_electron
    tracepy  = tp.py_electron
...
save,true,x,y,px,py,pz,eta,radn,radt,gamma,filename='./traceqed.sav'
