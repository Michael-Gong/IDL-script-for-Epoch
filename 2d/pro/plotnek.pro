...
density=b
b=ekbar*density/(1.1e27*0.51*1.6e-13)
set_plot,'ps'
for n=0,nspecie-1 do begin
for i=0,(endtime-starttime)/interval do begin
device,filename='./eps/nek-'+sname[n]+'-'+strtrim(string(starttime+interval*i),2)+'.'+filetype,$
...
axis,yaxis=1,ystyle=9,charthick=1.5,charsize=1.2,ytitle='on-axis nek [n_c*m_ec^2]',font=1
...
axis,yaxis=1,ystyle=9,charthick=1.5,charsize=1.2,ytitle='nek [n_c*m_ec^2]',font=1
device,/close
endfor
endfor
end
